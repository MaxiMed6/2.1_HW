...
def update_record(conn, id_val, new_name):
    cur = conn.cursor()
    cur.execute("UPDATE test_table SET name = %s WHERE id = %s", (new_name, id_val))
    conn.commit()
    cur.close()

def delete_record(conn, id_val):
    cur = conn.cursor()
    cur.execute("DELETE FROM test_table WHERE id = %s", (id_val,))
    conn.commit()
    cur.close()
