...
for outer_loop in range(total_cars):
    for inner_loop in range(total_cars - 1):
        if filtered_cars[inner_loop][1][4] > filtered_cars[inner_loop + 1][1][4]:
            filtered_cars[inner_loop], filtered_cars[inner_loop + 1] = filtered_cars[inner_loop + 1], filtered_cars[inner_loop]


for car in filtered_cars[0:5]:
    car_name, car_info = car
    color, year, engine_volume, car_type, price = car_info
    print(f"{car_name}: {color}, {year}, {engine_volume}L, {car_type}, ${price}")